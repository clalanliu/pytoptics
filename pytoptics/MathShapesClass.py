...
def even_asphere(x, y, E):
    ...
    for i in range(1, 9):
        if (E[(i - 1)] != 0):
            Z = (Z + (E[(i - 1)] * torch.power(r, ((2.0 * i*1.0) * 1.0))))
    return Z
...
class error_map__surf():
    ...
    def __init__(self, xValues, yValues, zValues, SPACE):
        ...
        """
        TG = SPACE
        LGMX = torch.max(yValues)
        LGMN = torch.min(yValues)
        LG = (LGMX - LGMN)
        NPG = int((1 + (LG / TG)))
        s = 40
...
def zernike_polynomials(term, ro, theta, Zern_pol, z_pow):
    ...
    L = len(ct)
    for i in range(0, L):
        NR = ((ct[i] * torch.power(ro, pot[i])) + NR)
    if (par == 1):
        S = (raiz * NR)
    if (par == 3):
        S = ((raiz * NR) * torch.cos((m * theta)))
    if (par == 2):
        S = ((raiz * NR) * torch.sin((m * theta)))
    return S
...
def r_zern(m, n):
    ...
    for s in range(0, (int(ls) + 1)):
        V1 = (torch.power((- 1), s) * np.math.factorial((n - s)))
...
def zernike_expand(L):
    ...
    """
    cont = 0
    Z = torch.tensor([0, 0, 0, 0, 0]).to(device)
    n = L
    m = L
    for i in range(0, n):
        if (cont >= L):
